...
import {
	SOQLQueryResultSchema,
	type SOQLQueryResult as SOQLQueryResultT,
} from "../../schemas";
...
export type SOQLQueryResult<T extends SOQLRecord = SOQLRecord> = Omit<
	SOQLQueryResultT,
	"records"
> & {
	records: T[];
...
export function useSOQLInfiniteQuery<T extends SOQLRecord = SOQLRecord>(
	...
	>({
		...
		queryFn: async ({ pageParam }) => {
			...
			return client.get(endpoint, { schema: SOQLQueryResultSchema }) as Promise<
				SOQLQueryResult<T>
			>;
		},
